...
// Curated, easily-distinguishable palette for admin-assigned employee colors.
// Only one green so employees don't keep landing on the same shade by default.
var EMP_COLOR_CHOICES = ["#EC4899","#F59E0B","#3B82F6","#EF4444","#8B5CF6","#14B8A6","#F97316","#0EA5E9","#DC2626","#7C3AED","#059669","#D97706"];
...
function overrideKey(weekOff, day, eid) { return dayToISODate(weekOff, day)+"_"+eid; }
...
function Welcome({ onLogin, onSignup, accounts, authError }) {
 ...
 async function doLogin() {
  ...
  setLoggingIn(true);
  var { data, error } = await supabase.auth.signInWithPassword({ email: emailClean, password: pw });
  setLoggingIn(false);
  if (!error && data.user) {
    // Real session is now live — App's onAuthStateChange listener picks this up
    // and loads the matching profile automatically, so no local state to set here.
    writeAudit("LOGIN_SUCCESS", emailClean, data.user.id);
  } else {
...
function App() {
 ...
 useEffect(function(){
  supabase.auth.getSession().then(function(res){
   ...
   if (session) { loadProfile(session.user); } else { setAuthLoading(false); }
  });
 ...
 useEffect(function(){
  ...
  supabase.from("shift_overrides").select("*")
   ...
   .then(function(res){
    if (res.data) {
     setShiftOverrides(function(p){
      var n = {...p};
      res.data.forEach(function(row){
       n[row.work_date+"_"+row.employee_id] = { start: row.start_time, end: row.end_time };
      });
      return n;
     });
 ...
 var [colorPickerFor, setColorPickerFor] = useState(null);
 ...
 var [openForm, setOpenForm] = useState({ day:DAYS[0], shift:"", role:"", note:"" });
  ...
  var [shiftOverrides, setShiftOverrides] = useState({}); // {ISOdate_eid: {start,end}} — loaded from Supabase below
 ...
 function colorFor(id) {
  var e = emps.find(function(x){ return String(x.id)===String(id); });
 ...
 async function setEmpColor(emp, color) {
  ...
  var res = await supabase.from("employees").update({ color: color }).eq("id", emp.id);
  if (res.error) { console.error("color update failed:", res.error); showT("Couldn't save color — check connection","error"); }
 }
 ...
 async function resetPassword(acc) {
  var pw = window.prompt("New password for "+acc.email+" (min 8 characters):");
  if (!pw) return;
  if (pw.length < 8) { showT("Password must be at least 8 characters","error"); return; }
  var sessionRes = await supabase.auth.getSession();
  var accessToken = sessionRes.data.session ? sessionRes.data.session.access_token : null;
  if (!accessToken) { showT("Not signed in — can't reset password","error"); return; }
  try {
   var apiRes = await fetch("/api/reset-employee-password", {
    method: "POST",
    headers: { "Content-Type": "application/json", "Authorization": "Bearer " + accessToken },
    body: JSON.stringify({ email: acc.email, newPassword: pw }),
   });
   var apiData = await apiRes.json();
   if (!apiRes.ok) { console.error("password reset failed:", apiData); showT("Reset failed: " + apiData.error, "error"); }
   else { showT("Password reset for "+acc.email); }
  } catch (err) {
   console.error("password reset request failed:", err);
   showT("Couldn't reach the server to reset password","error");
  }
 }
 ...
 // Picks shifts for one employee across the week, respecting availability, PTO,
 // per-day shift restrictions, and max weekly hours. Shared by the bulk
 // Auto-Generate button and the per-employee "click their tag" fill.
 function pickShiftsForEmployee(emp, ptoBlockedDays) {
  var result = {};
  var hoursUsed = 0;
  var shiftCycle = shiftDefs.map(function(s){ return s.id; });
  DAYS.forEach(function(d) {
   if (ptoBlockedDays && ptoBlockedDays[d]) return; // approved PTO — never override
   if (emp.avail.indexOf(d) < 0) return; // not available this day
   var allowedShiftIds = (emp.shiftAvail && emp.shiftAvail[d] && emp.shiftAvail[d].length > 0)
    ? shiftCycle.filter(function(sid){ return emp.shiftAvail[d].indexOf(sid) >= 0; })
    : shiftCycle;
   if (allowedShiftIds.length === 0) return;
   var fitting = allowedShiftIds.filter(function(sid){ return hoursUsed + shiftHours(sid, shiftDefs) <= emp.max; });
   if (fitting.length === 0) return;
   var chosen = fitting[Math.floor(Math.random() * fitting.length)];
   result[d] = chosen;
   hoursUsed += shiftHours(chosen, shiftDefs);
  });
  return result;
 }
 ...
 var picks = pickShiftsForEmployee(emp, ptoBlocked[emp.id]);
 if (Object.keys(picks).length === 0 && ptoBlocked[emp.id]) {
  if (skipped.indexOf(emp.name) < 0) skipped.push(emp.name);
 }
 DAYS.forEach(function(d) {
  if (ptoBlocked[emp.id] && ptoBlocked[emp.id][d] && skipped.indexOf(emp.name) < 0) skipped.push(emp.name);
  if (picks[d]) gen[d][emp.id] = picks[d];
 });
 });
 ...
 // Fills one employee's whole week using the same availability/PTO/max-hours
 // logic as Auto-Generate, without touching anyone else's shifts. Used when a
 // manager is building the schedule day-by-day and clicks a consistent
 // employee's tag instead of assigning each shift by hand.
 function fillEmployeeWeek(emp) {
  if (isPastWeek) { showT("Past schedules are locked","error"); return; }

  var weekStart = getWeekStart(weekOff);
  var ptoBlockedDays = {};
  ptos.forEach(function(req) {
   if (req.status !== "approved" || req.eid !== emp.id) return;
   var startStr = req.startDate;
   var endStr = req.endDate || req.startDate;
   DAYS.forEach(function(d, di) {
    var dayDate = new Date(weekStart);
    dayDate.setDate(weekStart.getDate() + di);
    var dateStr = isoDate(dayDate);
    if (dateStr >= startStr && dateStr <= endStr) ptoBlockedDays[d] = true;
   });
  });

  var picks = pickShiftsForEmployee(emp, ptoBlockedDays);
  ...
  setSched(function(p) {
   ...
   DAYS.forEach(function(d) {
    var dayObj = {...(wk[d]||{})};
    if (picks[d]) dayObj[emp.id] = picks[d]; else delete dayObj[emp.id];
    wk[d] = dayObj;
   });
   n[weekOff] = wk;
   return n;
  ...
  DAYS.forEach(function(d) {
   if (picks[d]) syncScheduleCell(weekOff, d, emp.id, picks[d]);
 ...
 * { box-sizing:border-box; margin:0; padding:0; }
 ...
 <div style={{ width:26, height:26, borderRadius:"50%", background:colorFor(user.id), display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, fontWeight:700, color:"white" }}>{user.name[0]}</div>
 ...
 <div style={{ display:"flex", alignItems:"center", gap:8 }}>
 <div style={{ width:30, height:30, borderRadius:"50%", background:colorFor(user.id), display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:700, color:"white", flexShrink:0 }}>{user.name[0]}</div>
         ...
         {onS.map(function(emp){
           ...
           return (
             <div key={emp.id} style={{ ...CARD, display:"flex", alignItems:"center", gap:12, padding:"12px 16px", border:"1px solid "+(conf?T.danger:sc.border||T.border) }}>
               <div style={{ width:34,height:34,borderRadius:"50%",background:colorFor(emp.id),display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,color:"white",flexShrink:0 }}>{initials(emp.name)}</div>
               <div style={{ flex:1, minWidth:0 }}>
           ...
           <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
             {offS.map(function(emp){ return (
               <div key={emp.id} style={{ display:"flex", alignItems:"center", gap:5, padding:"4px 9px", borderRadius:20, background:T.bg, border:"1px solid "+T.border }}>
                 <div style={{ width:16,height:16,borderRadius:"50%",background:colorFor(emp.id),display:"flex",alignItems:"center",justifyContent:"center",fontSize:7,fontWeight:700,color:"white" }}>{initials(emp.name)}</div>
 ...
 <div onClick={function(){if(!isPastWeek)fillEmployeeWeek(emp);}} title={isPastWeek?"":"Click to fill "+emp.name.split(" ")[0]+"'s week (availability, PTO respected)"} className={isPastWeek?"":"hv"} style={{ display:"flex", alignItems:"center", gap:9, cursor:isPastWeek?"default":"pointer", borderRadius:8, padding:"2px 4px", margin:"-2px -4px" }}>
 <div style={{ width:26, height:26, borderRadius:"50%", background:colorFor(emp.id), display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, fontWeight:700, color:"white", flexShrink:0 }}>{initials(emp.name)}</div>
 <div>
 ...
 {shift && shiftOverrides[overrideKey(weekOff,d,emp.id)] && (
   <div onClick={function(e){
     ...
     supabase.from("shift_overrides").delete()
      .eq("property_id", user.property_id).eq("work_date", dayToISODate(weekOff,d)).eq("employee_id", emp.id)
      .then(function(res){ if (res.error) { console.error("override delete failed:", res.error); showT("Removed locally, but the server save failed","error"); } });
 ...
 <button onClick={function(){setColorPickerFor(colorPickerFor===emp.id?null:emp.id);}} title="Change color" style={{ width:38, height:38, borderRadius:"50%", background:colorFor(emp.id), display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, fontWeight:700, color:"white", flexShrink:0, border:"none", cursor:"pointer", padding:0 }}>{initials(emp.name)}</button>
 <div style={{ flex:1 }}>
 ...
 <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:11, padding:"8px 9px", background:T.bg, borderRadius:7 }}>
 {EMP_COLOR_CHOICES.map(function(c){
 return <button key={c} onClick={function(){setEmpColor(emp,c);}} style={{ width:20, height:20, borderRadius:"50%", background:c, border:emp.color===c?"2px solid "+T.text:"2px solid transparent", cursor:"pointer", padding:0 }} />;
 })}
 </div>
 )}
 {acc && (
 ...
 <button onClick={function(){resetPassword(acc);}} style={{ fontSize:10, color:T.muted, background:"transparent", border:"1px solid "+T.border, borderRadius:20, padding:"1px 7px", cursor:"pointer" }}>Reset PW</button>
 ...
 <div style={{ display:"flex", alignItems:"center", gap:9 }}>
 <div style={{ width:28, height:28, borderRadius:"50%", background:colorFor(e.id), display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, fontWeight:700, color:"white", flexShrink:0 }}>{initials(e.name)}</div>
 <div>
                            ...
                            return (
                              <div key={e.id} style={{ ...CARD, padding:14, border:"1px solid "+(isMe?dept.color:T.border), background:isMe?(dept.color+"0D"):T.surface }}>
                                <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
                                  <div style={{ width:30, height:30, borderRadius:"50%", background:colorFor(e.id), display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, fontWeight:700, color:"white", flexShrink:0 }}>{initials(e.name)}</div>
                        ...
                        return (
                          <div key={item.emp.id} style={{ ...CARD, border:"1px solid #6EE7B7", background:T.successL }}>
                            <div style={{ display:"flex", alignItems:"center", gap:9, marginBottom:8 }}>
                              <div style={{ width:32, height:32, borderRadius:"50%", background:colorFor(item.emp.id), display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:700, color:"white" }}>{initials(item.emp.name)}</div>
                              <div>
        ...
        <div style={{ ...CARD, maxWidth:340, width:"100%", padding:24, overflow:"hidden" }} onClick={function(e){e.stopPropagation();}}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            <button onClick={function(){
              ...
              supabase.from("shift_overrides").upsert({
               property_id: user.property_id,
               work_date: dayToISODate(weekOff, editOverride.day),
               employee_id: editOverride.eid,
               start_time: editOverride.start,
               end_time: editOverride.end,
              }, { onConflict: "property_id,work_date,employee_id" }).then(function(res){
               if (res.error) { console.error("override save failed:", res.error); showT("Saved locally, but the server save failed","error"); }
              });
